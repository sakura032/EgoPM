...
| **本 CR 的 v2** | **最多 5 条 Atom 一个自适应 package** | **¥89.84** | **5.11 小时** | **唯一允许实施的正式方案** |

v2 在相同覆盖和模型下，将请求数从 370799 降至 74160；按保守上界，费用降低约 82.3%，
时长下界降低约 84.4%。这不是删减样本或降低最终 Cue 数据要求，而是消除重复提示词、完整
...
- 不采用 Batch File、上下文缓存或任何其他调用通道；它们不属于本次正式实施范围。
...
| **已采用 v2：最多 5 条/包** | **74160** | **259333645** | **47462272** | **¥89.84** | **5.11 小时** |
