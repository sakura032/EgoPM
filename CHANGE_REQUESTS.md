...
- 状态：**用户已确认终止 v2.2 第 1 波 Batch；v3 无 API 配置、执行器和独立 QA 实施中。**
...
- 成本：按北京实时公开价输入 `¥0.2/M`、输出 `¥0.8/M`，v3 正式 Source 流式预检得到输入代理
  `55480053`、输出上界 `35596704 token`，单次保守上界为 `¥39.5733738`。这低于现有 `¥40`，
  但余量仅约 `¥0.43`；正式执行前仍必须新建本机未提交授权，重新绑定 v3 协议 SHA，并明确
  是否接受无重试预算。若最大两次重试均耗尽，极端上界约为 `¥118.7201214`。
