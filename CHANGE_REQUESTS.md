...
- 状态：**已提出，待 T0 冻结新的 Schema、协议版本、配置哈希与迁移计划；在此之前不得继续 Wave 1。**
- 动机：仅在 `visible_text` 中匹配会遗漏可追溯地存在于同一 Source Atom 的 `transcript` 或 `dense_caption` 证据；将三字段拼接匹配会丢失证据来源，且可能跨字段形成并非任一原文连续片段的假证据。
- 目标：每条最终 Cue 必须新增 `supporting_text_field`，枚举为 `transcript`、`dense_caption`、`visible_text`。模型在紧凑推理对象中返回对应字段码，程序将其确定性回填为最终字段；`supporting_text_span` 只可在该 Atom 的该字段内验证。
- 字段内匹配采用确定性归一化：对候选片段与被选字段同时执行 Unicode `NFKC`、`casefold`，并删除 Unicode 空白、类别 `P*` 的标点及类别 `Cf` 的格式控制字符；归一化后候选不得为空，且必须仍为被选字段的连续子串。其他字符及其顺序必须完全保留，禁止释义、翻译、实词增删、数值或单位变化及重排。
- 非目标：不改变 Source Atom、split、每 package 最多 5 Atom、八波计划、预算上限、无正文响应规则或最终 Cue 的其余语义字段；不接受跨 Atom 或跨字段拼接证据。

### 兼容性影响与迁移

1. 此变更新增最终 Schema 必填字段，并修改紧凑推理 Schema、提示词、模型登记/协议哈希、T2 展开逻辑与 T4 独立验证，属于不兼容的协议与 Schema 变更；需提升相应语义版本并重新计算协议 SHA256。
2. `source_text` 的既有受控来源保持为 `visible_text`；`supporting_text_field` 是实际证据位置，T4 必须从映射 Atom 的声明字段重新取证，而不得把 `source_text`、`transcript`、`dense_caption` 拼接为可匹配语料。
3. `realtime_v7_01` 已有的 19 个成功 package 不含新必填字段，package 015 亦为不可重发隔离。因此整个 v7 运行只能作为审计输入，不能与新协议产物合并、恢复或重跑；新协议必须使用新的 run ID 和新的本地授权绑定。已发生的 `¥0.0053176` 仅保留在 v7 无正文账本中。
4. 先前仅放宽 `visible_text` 格式匹配的未提交实现不得单独发布为 v3.3.0 行为变更；它可作为本 CR 的共享归一化基础，经 Schema/协议冻结后再与字段选择一并验收。

### 建议实施与验收门

| 阶段 | 负责人 | 必需工作 |
| --- | --- | --- |
| 冻结 | T0 | 更新治理状态、最终/推理 Schema、协议版本与哈希边界；明确紧凑字段码和新 run ID；更新授权模板与迁移说明。 |
| 生产者 | T2 | 更新提示词、请求 Schema、解析与字段内匹配；对缺失、未知或空归一化字段安全隔离；不得保存原始响应。 |
| 独立 QA | T4 | 从 Source Atom 的声明字段独立取证，并验证字段枚举、连续性、归一化、血缘、跨字段/跨 Atom 拒绝与新旧运行隔离。 |
| 验收 | T0 | 完整无 API 测试、协议哈希重算、冻结 Source 预检、预算重新授权；仅随后由用户执行新的单行 API 命令。 |

必须新增至少以下测试：三种字段各自的纯格式差异通过；实质改写、数值/单位变化、非连续重排、空归一化候选、未知字段码、字段与片段不匹配、跨字段借用、跨 Atom 借用均拒绝；T4 与 T2 的匹配结果一致。任何既有 v7 package 都不得被该变更触发重发。

### 补充冻结：位置证据与逐项恢复

本补充取代上文中模型回传 `supporting_text_span` 的实现细节，保留 `supporting_text_field` 的最终 Schema 目标。

1. 紧凑推理对象必须回传 `f`、`start` 与 `end`；`f` 是 `T`、`D`、`V` 中的一个字段码，`start`/`end` 是该字段 Python/Unicode code-point 序列的零基、左闭右开偏移。模型不再回传 `x`。程序只在 `0 <= start < end <= len(Atom[f])` 时，确定性切出最终 `supporting_text_span`。
2. 最终 Cue 的 `supporting_text_span` 因此逐字来自声明字段；T2/T4 仍须对切出片段和声明字段执行 `NFKC`、`casefold`、删除 Unicode 空白、`P*` 与 `Cf` 的归一化连续性检查，作为对格式边界和实现一致性的独立门。不得删除字母、数字、符号或改变任何保留字符的顺序。
3. 一个多 Atom package 的响应必须逐项验证和原子落盘。通过项立即写入可哈希的结果片段；失败项只记录 `atom_id`、`item_index`、安全错误码、JSON 路径、尝试次数和无正文请求身份。不得将失败项的原始响应、原始请求或字幕正文写入账本、失败标记或审计队列。
4. 首次失败项可按冻结上限发起仅含该 Atom 的修复请求；本 CR 将上限冻结为每项 **1** 次。修复请求与原 package 分别记账，绝不重付同 package 其余 Atom 的成本。修复仍失败时写入独立人工/二级模型审计队列，并允许 Wave 继续处理其他 package。
5. 审计队列不是成功 Cue。任何未解决队列项都必须阻断正式 `CUE_LIBRARY_SUCCESS`、Cue QA `DONE` 与下游 Seed 门；它只是不再阻断同一 Wave 的其余调度。二级模型审计的模型、预算、授权和输出规则必须在另行批准的执行子协议中冻结，不能由本次 API 恢复命令隐式调用。
6. 此补充再次改变紧凑 Schema、提示词、解析、结果片段/账本结构、恢复语义、T4 QA 和协议哈希。最终 Cue Schema 仍升为 `v1.1.0`；Cue 执行协议升级为 `v3.5.0`，新 run 必须使用 `realtime_v8_01` 或另一未使用的 v3.5 run ID，并以总额 `¥80` 的新本地授权绑定新协议 SHA。旧运行已发生费用仅作审计与实际成本记录，不得借由新 run 隐藏或删除。
