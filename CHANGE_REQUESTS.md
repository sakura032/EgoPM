...
- 不将上下文缓存或 Batch File 的未验证折扣写入实时生产预算。
...
对当前冻结来源的只读模拟中，5 条上限产生 74160 个请求 package：741 个完整 shard 各有
100 个 package，最后一个 299 Atom shard 有 60 个 package，其中最后一个 package 有 4 条。
最大的模拟请求为 11505 UTF-8 字节，因此当前来源会在 24000 字节保护线内运行；该结论必须在
...
| --- | ---: | ---: | ---: | ---: | ---: |
| 已冻结逐 Atom v1 | 370799 | 1779240532 | 189849088 | ¥507.73 | 32.82 小时 |
| v2，最多 3 条/包 | 123600 | 378830125 | 47462272 | ¥113.74 | 7.11 小时 |
| v2，最多 4 条/包 | 92700 | 304144825 | 47462272 | ¥98.80 | 5.86 小时 |
| **v2，最多 5 条/包** | **74160** | **259333645** | **47462272** | **¥89.84** | **5.11 小时** |

若每个 package 都耗尽两次额外重试，5 条包的极保守上界约为 `¥269.51`。该数字不是批准预算；
用户必须在正式执行前确认可接受的总预算、失败重跑上限、是否允许夜间连续运行，以及是否采用
Batch File。
...
`qwen3.7-flash` 在北京支持 Batch File。该通道对成功请求的输入和输出按实时价格 50% 计费；
若在 v2 五条包已完成质量验证后单独采用，模拟上界可进一步降至约 `¥44.92`。但 Batch File 是
异步任务，要求上传请求 JSONL、轮询任务、下载结果/错误文件并按 `custom_id` 匹配，且不与
上下文缓存叠加计费。因此它是第二阶段的独立 CHANGE_REQUEST，不得在本 CR 的实时恢复实现中
隐式切换。依据见[批量推理说明](https://help.aliyun.com/zh/model-studio/batch-inference)。

#### 上下文缓存：仅做计量验证，不预扣预算

平台的 OpenAI 兼容接口支持上下文缓存，但缓存有效期、最小前缀、`response_format` 是否计入可
命中前缀及实际账单字段均须以独立小试确认。当前不将缓存折扣纳入本 CR 的成本表，也不以缓存
为通过生产预算的前提。若要采用，须先冻结缓存标记策略、5 分钟存活期处理、命中证明和未命中
回退预算。依据见[上下文缓存说明](https://help.aliyun.com/zh/model-studio/context-cache)。
...
| 4 | 用户 | 确认总预算、实时或 Batch File、运行时间窗和失败重跑范围 | 明确生产授权 | 含糊地默认授权 |
...
- 边界文本、超长文本、3/4/5 条包和末尾 4 条 package 均符合字节保护线和输出上限。
...
本文件已经固定“最多 5 条/包”为默认实施方向。下一步是 T0 依据本 CR 冻结 v2 配置和验收字段，
再由 T2/T4 仅以无 API 合成 fixture 实现及测试。正式模型调用仍等待用户的预算、执行范围和分批
方式确认。
