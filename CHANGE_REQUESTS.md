...
- 状态：**仅完成无 API 设计与保守核算；未冻结配置、未创建推理 Schema、未写代码、未调用 API**。
...
基线仍是 74160 个五条 package、311023165 UTF-8 输入字节代理。若上述两个输入体积门都达到，
每个 package 最多节省 `444 + 900 = 1344` 字节代理，共节省 `99671040`；因此新的输入代理上界为
`211352125`。这不是实测 Token，正式实现后必须重新跑冻结 Source 预检。

| 路径 | 输入代理上界 | 输出上界 | 保守费用上界 |
| --- | ---: | ---: | ---: |
| 原始 v1：逐 Atom 实时请求 | 1779240532 | 189849088 | ¥507.7273768 |
| **拟采用 v2.2：Batch File、短码协议、96 token/Atom** | **211352125** | **35596704** | **¥35.3738941** |

设计目标相对原始 v1 约降 `93.0%`。费用使用北京 Batch File
输入 `¥0.1/M`、输出 `¥0.4/M` 的当前公开价格快照；Batch 仍只对成功请求计费。正式预算应向上
取整为 **¥35.38**，并在提交任何远端任务前以最新价格与实际 token usage 重新确认。
